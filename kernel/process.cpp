...
process::process(kernel &p_kernel, terminal *p_terminal, addr_t const p_program) : m_kernel(p_kernel), m_used_blocks(), m_heap_address(USER_HEAP_ADDRESS), m_terminal(p_terminal), m_input_buffer(), m_sys_read_buffer(nullptr), m_sys_read_count(0) {
  this->id = 1;
